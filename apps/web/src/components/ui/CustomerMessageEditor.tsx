  ...
  return (
    <div className="bg-black/20 rounded-lg p-4 border border-gray-800">
      <div className="flex justify-between items-center mb-3">
        <label className="text-sm font-medium text-gray-300">Edit Customer Message</label>
        <div className="relative group">
          <button className="text-xs text-primary flex items-center gap-1 hover:text-white transition-colors">
            <Languages size={14} /> Templates
          </button>
          <div className="absolute right-0 top-6 w-64 bg-[#161622] border border-gray-800 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-opacity z-10 p-2">
            {templates.map(t => (
              <div 
                key={t.name}
                className="text-xs p-2 hover:bg-white/5 cursor-pointer rounded text-gray-300"
      ...
      <textarea 
        ...
        className="w-full h-24 bg-black/40 border border-gray-700 rounded-lg p-3 text-sm text-gray-200 focus:outline-none focus:border-primary resize-none mb-3"
      />
